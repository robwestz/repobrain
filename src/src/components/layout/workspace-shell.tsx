...
import {
  ModelSelector,
  type LLMSelection,
} from "@/src/components/workspace/model-selector";
 ...
 * - selected LLM provider/model (persisted in localStorage via ModelSelector)
 */
export function WorkspaceShell({
  ...
  const [repos, setRepos] = useState<RepoInfo[]>(
    initialRepos ?? (initialRepo ? [initialRepo] : []),
  );
  const [showRepoPicker, setShowRepoPicker] = useState(false);

  // Selected LLM provider/model — persisted in localStorage by ModelSelector
  // Currently held as context; future: pass to chat API calls
  const [llmSelection, setLlmSelection] = useState<LLMSelection | null>(null);
  void llmSelection; // available for future wiring to chat/LLM calls
    ...
    (path: string) => {
      setOpenTabs((prev) => {
        ...
        if (activeTabPath === path) {
          setActiveTabPath(
            next.length > 0 ? next[next.length - 1].path : null,
          );
          setHighlightRange(null);
        }
        return next;
  ...
  const repoCountLabel =
    repos.length > 1 ? ` (+${repos.length - 1} more)` : "";
      ...
      <div className="flex flex-1 flex-col min-w-0">
        ...
        <header className="flex h-12 shrink-0 items-center justify-between gap-4 border-b px-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <ModelSelector onChange={setLlmSelection} />
          </div>
        </header>
        ...
        <PanelGroup direction="horizontal" className="flex-1 overflow-hidden">
          ...
          <Panel defaultSize={50} minSize={30}>
            <div className="flex h-full flex-col">
              ...
              {/* Main content area */}
              {!repo && (
                <EmptyState onConnectRepo={() => setShowRepoPicker(true)} />
              )}
                ...
                <div className="flex flex-1 flex-col items-center justify-center gap-4 p-8 text-center">
                  <div className="max-w-sm">
                    ...
                    <p className="mt-2 text-sm text-[var(--muted-foreground)]">
                      {repo.errorMessage ??
                        "An unknown error occurred while cloning the repository."}
                    </p>
                  </div>
                  <button
                ...
                <div className="flex flex-1 items-center justify-center p-8">
                  <div className="text-center">
                    <p className="text-sm text-[var(--muted-foreground)]">
                      Select a file from the tree or ask a question to get
                      started
                    </p>
                  </div>
                </div>
                ...
                <div className="flex-1 overflow-hidden">
                  {activeTab.loading ? (
                    ...
                    <div className="flex h-full flex-col items-center justify-center gap-2 p-4 text-center">
                      ...
                      <button
                        onClick={() =>
                          openFile(activeTab.path, activeTab.language)
                        }
